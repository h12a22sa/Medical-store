    ...
    <div className="space-y-5 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between rounded-3xl border border-slate-200/80 bg-white p-6 shadow-xs dark:border-slate-800/80 dark:bg-slate-900">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Financial accounting, expense logging, and gross vs net margin tracking
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
            ...
            <button
              ...
              className={`rounded-xl px-4 py-2 text-xs font-bold transition-all cursor-pointer ${
                activeTab === 'profit-loss'
                  ? 'bg-white text-emerald-800 shadow-xs dark:bg-slate-700 dark:text-emerald-300'
                  : 'text-slate-600 hover:text-slate-900 dark:text-slate-400'
              }`}
            >
              P&L Statement
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-2xl bg-rose-600 px-4 py-2.5 text-xs font-bold text-white shadow-md shadow-rose-600/20 hover:bg-rose-500 transition-colors cursor-pointer"
          >
      ...
      {activeTab === 'expenses' ? (
        <>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div className="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-xs dark:border-slate-800/80 dark:bg-slate-900">
              ...
              <div className="mt-1 text-[11px] text-slate-500 font-semibold">{expenses.length} Entries Logged</div>
            </div>

            <div className="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-xs dark:border-slate-800/80 dark:bg-slate-900">
              <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Highest Expense Category</span>
              <p className="mt-2 text-base font-bold text-slate-900 dark:text-white">
                {categoryBreakdown[0]?.category || 'None'}
              </p>
              <div className="mt-1 text-[11px] font-mono font-semibold text-slate-500">{formatCurrency(categoryBreakdown[0]?.total || 0)}</div>
            </div>

            <div className="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-xs dark:border-slate-800/80 dark:bg-slate-900">
              <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Payment Reconciliation</span>
              <p className="mt-2 text-sm font-bold text-slate-800 dark:text-slate-200">
                Cash Drawer & Online Accounts
              </p>
                  ...
                  {expenses.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="py-14 text-center text-slate-400">
                        <p className="font-bold text-xs text-slate-600 dark:text-slate-300">No expenses recorded yet</p>
                      </td>
                    </tr>
                  ) : (
                    expenses.map(e => (
                      <tr key={e.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                        ...
                        <td className="py-3.5 px-3">
                          <span className="inline-block rounded-lg bg-rose-50 px-2.5 py-0.5 text-[10px] font-bold text-rose-700 dark:bg-rose-950/60 dark:text-rose-300">
                            {e.category}
                          </span>
                        </td>
        ...
        <div className="space-y-6">
          <div className="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-xs dark:border-slate-800/80 dark:bg-slate-900 max-w-3xl mx-auto space-y-6">
            <div className="text-center pb-4 border-b border-slate-100 dark:border-slate-800">
              <h2 className="text-lg font-black text-slate-900 dark:text-white">Commercial Statement of Profit & Loss</h2>
              <p className="text-xs text-slate-500 mt-0.5">Awan Medical Store • Real-time Financial Ledger</p>
            </div>

            {/* Income Section */}
            <div className="space-y-3">
              <h3 className="text-xs font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">1. Revenue & Sales</h3>
              <div className="rounded-2xl bg-emerald-50/40 p-4 border border-emerald-100/80 dark:bg-emerald-950/20 dark:border-emerald-900/40 space-y-2.5 text-xs">
                ...
                <div className="flex justify-between text-slate-600 dark:text-slate-400">
                  ...
                  <span className="font-mono text-rose-600 font-semibold">- {formatCurrency(totalCostOfGoods)}</span>
                </div>
                <div className="flex justify-between border-t border-emerald-200/80 pt-2.5 text-sm font-black text-emerald-900 dark:text-emerald-200 dark:border-emerald-800">
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-2xl bg-rose-50/40 p-4 border border-rose-100/80 dark:bg-rose-950/20 dark:border-rose-900/40 space-y-2.5 text-xs">
                ...
                <div className="flex justify-between border-t border-rose-200/80 pt-2.5 text-sm font-black text-rose-900 dark:text-rose-200 dark:border-rose-800">
            ...
            {/* Final Net Profit */}
            <div className="rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 p-5 text-white shadow-md shadow-emerald-600/20 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs opacity-90 font-medium">Net Profit Margin: {netMargin.toFixed(1)}%</p>
              </div>

              <div className="text-right">
                <button
                  onClick={() => window.print()}
                  className="rounded-2xl bg-white px-4 py-2 text-xs font-bold text-emerald-900 shadow-xs hover:bg-emerald-50 transition-colors cursor-pointer"
                >
                  Print P&L
                </button>
              </div>
            ...
            <form onSubmit={handleSaveExpense} className="mt-4 space-y-3.5 text-xs">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-2xl border border-slate-200 bg-slate-50/70 px-3.5 py-2.5 font-mono font-bold text-sm dark:border-slate-700 dark:bg-slate-800 dark:text-white focus:border-emerald-500 focus:outline-none"
                />
              </div>
              ...
              <div className="flex justify-end gap-2.5 pt-3 border-t border-slate-100 dark:border-slate-800">
                ...
                <button
                  type="submit"
                  className="rounded-2xl bg-rose-600 px-5 py-2 font-bold text-white shadow-md shadow-rose-600/20 hover:bg-rose-500 transition-colors cursor-pointer"
                >
                  Save Expense
                </button>
              </div>
